        ...
        with st.spinner("🧠 Analyzing question, retrieving context, generating answer, and evaluating..."):
            ...
            try:
                ...
                if not OPENAI_API_KEY: # Check if the OpenAI API key is available
                    ...
                    entities = baseline_model.extract_entities(question_to_analyze)
                    if entities==[]:
                        st.warning("No entities were found in the question. "
                                   "The LLM will attempt to answer based on its general knowledge.")
                        logging.warning("LLM Generation: No entities found in the question.")
                    context_texts_for_llm = []
                    for entity in entities:
                        context_texts_for_llm.extend(
                                baseline_model.retrieve_relevant_paths(query=question_to_analyze, seed_entity_value=entity, k_hop=5, num_epochs=1, top_n_results=5)
                            )
