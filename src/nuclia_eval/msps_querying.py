...
class MSPN_Search:
    def __init__(self, embedding_model:str = 'sentence-transformers/all-MiniLM-L6-v2'):
        ...
        self.gnn_model = SimpleMPNN(aggregation='mean', num_layers=5)
        self.entity2id = {}
        self.relation2id = {}
            ...
            for target_node_local_idx in range(N_sub):
                ...
                if path_nodes_display_names:
                    ...
                    for i in range(len(path_nodes_display_names) - 1):
                        ...
                        para_id = self.relation2metadata.get((head, rel, tail))
    ...
    def query_knowledge_graph(self, question: str, k_hops: int = 2, top_n_results: int = 5, model_override: Optional[str] = None ) -> List[Dict[str, Any]]:
        if not self.entity2id:
            logger.error("No entities loaded. Please call ingest_data first.")
            FALLBACK_ANSWER["question"] = question
        ...
        top_scores = self.retrieve_relevant_paths(sub_graph,subset,question_emb_norm, seed_local_indices)

        # If no paths found, return a fallback answer
        if not top_scores:
        ...
        for item in top_scores:
            unique_para_ids.update(item['para_ids'])  # Collect unique paragraph IDs
